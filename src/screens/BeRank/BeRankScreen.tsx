...
import type { CompletedChallengeHistoryEntry, PurchaseCategory } from '../../utils/beRankSimulation';
...
const BeRankScreen: FC<BeRankScreenProps> = ({
    currentNav,
    onNavChange,
    summary,
    rankingLabel,
            ...
            <div style={scrollContainerStyle}>
                ...
                <section style={sectionStyle}>
                    ...
                    <div style={statusCardStyle}>
                        ...
                        <div style={nextStepStyle}>
                            <span style={nextStepArrowStyle}>→</span> Prochaine étape : {summary.pointsToNextRank > 0 ? `Plus que ${summary.pointsToNextRank} pts pour le rang ${summary.nextRank}` : 'Vous avez atteint le rang maximum'}
                        </div>
                    </div>
                </section>
                ...
                <section style={sectionStyle}>
                    ...
                    <div style={simulatorCardStyle}>
                        ...
                        >
                            <span>Ajouter une opération BeRank</span>
                            <span aria-hidden="true">{isSimulatorOpen ? '−' : '+'}</span>
                        </button>

                        {isSimulatorOpen && (
                            <PurchaseSimulationForm
                                submitLabel="Confirmer l'achat"
                ...
                <section style={sectionStyle}>
                    ...
                    <div style={stackStyle}>
                        {challengeHistory.length > 0 ? (
                            challengeHistory.slice(0, 4).map((entry) => (
                                <div key={entry.id} style={historyCardStyle}>
                                    ...
                                    {entry.unlockedBenefit ? (
                                        <div style={historyBenefitStyle}>Récompense : {entry.unlockedBenefit}</div>
                                    ) : null}
                                </div>
                            ))
                        ) : (
                            <div style={historyEmptyStyle}>Les défis complétés apparaîtront ici automatiquement.</div>
                        )}
                    </div>
                </section>
...
const simulatorCardStyle: CSSProperties = {
    display: 'flex',
    flexDirection: 'column',
    gap: tokens.spacing.sm,
};

const simulatorToggleStyle: CSSProperties = {
    display: 'flex',
    alignItems: 'center',
...
const historyBenefitStyle: CSSProperties = {
    fontSize: '12px',
    color: tokens.colors.text.primary,
};
